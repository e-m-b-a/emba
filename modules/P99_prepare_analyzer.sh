...
P99_prepare_analyzer() {
  ...
  if [[ "${#ROOT_PATH[@]}" -gt 0 ]] && ! [[ -f "${CSV_DIR}"/p99_prepare_analyzer.csv ]]; then
    ...
    for lR_PATH in "${ROOT_PATH[@]}"; do
      ...
      lUNIQUE_FILES=$(find "${ROOT_PATH}" "${EXCL_FIND[@]}" -xdev -type f  -print0|xargs -0 -P 16 -I % sh -c 'md5sum % 2>/dev/null' | sort -u -k1,1 | cut -d\  -f3 | wc -l )
      lDIRS_EXT=$(find "${ROOT_PATH}" -xdev -type d | wc -l )
      lBINS=$(find "${ROOT_PATH}" "${EXCL_FIND[@]}" -xdev -type f  -print0|xargs -0 -P 16 -I % sh -c 'file %' | grep -c "ELF" || true)
