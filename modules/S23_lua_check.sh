...
S23_lua_check() {
  ...
  if [[ "${lS23_LUA_VULNS}" -gt 0 || "${lS23_LUA_ISSUES}" -gt 0 ]]; then
    sub_module_title "LUA Security results overview"
  ...
  for lQUERY_FILE in "${LUA_CGI_FILES_ARR[@]}"; do
    ...
    for lENTRY in "${lQUERY_ENTRIES_ARR[@]}"; do
      ...
      if grep "${lENTRY}" "${lQUERY_FILE}" 2>/dev/null | grep -E -q "io\.(p)?open"; then
        ...
        sed -i -r "s/.*io\.(p)?open.*/\x1b[32m&\x1b[0m/" "${lLOG_QUERYFILE}"
        lAI_PRIO=$((lAI_PRIO + 1))
      fi
    ...
    if [[ "${lISSUES_FILE}" -gt 0 ]]; then
      ...
      if [[ "${AI_OPTION}" -gt 0 ]]; then
        ...
        write_anchor_AI "${lAI_ANCHOR}" "${lLOG_QUERYFILE}"
      fi
    fi
  done
}
