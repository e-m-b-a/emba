...
run_init_test() {
  ...
  sed -i 's/.REF.*//' "${lLOG_FILE_INIT}" || print_error "[-] S115 error in sed command to replace REF in ${lLOG_FILE_INIT}"
  write_log "\\n-----------------------------------------------------------------\\n" "${lLOG_FILE_INIT}"
}
...
emulate_strace_run() {
  ...
  if [[ -f "${lLOG_FILE_STRACER}" ]]; then
    ...
    sed -i 's/.REF.*//' "${lLOG_FILE_STRACER}" || print_error "[-] S115 - sed command for removing REF entries failed in ${lLOG_FILE_STRACER}"
...
recover_local_ip() {
  ...
  if ! ifconfig eth0 | grep -q "${lIP_TO_CHECK}"; then
    ...
    ifconfig eth0 "${lIP_TO_CHECK}" up || true
  fi
}
