  ...
  for lFILE in "${FILE_ARR_LIMITED[@]}" ; do
    ...
    if [[ "${lK_VER}" =~ Linux\ version\ .* ]]; then
      ...
      if [[ ! -f "${lFILE}".elf ]]; then
        ...
        write_log "${lPACKAGING_SYSTEM};${lFILE:-NA};${lMD5_CHECKSUM:-NA}/${lSHA256_CHECKSUM:-NA}/${lSHA512_CHECKSUM:-NA};linux_kernel:$(basename "${lFILE}");${lK_VER:-NA};${lSTRIPPED_VERS:-NA};${lAPP_LIC:-unknown};${lAPP_MAINT:-NA};${lK_ELF:-NA};${lCPE_IDENTIFIER};${lPURL_IDENTIFIER};Linux Kernel" "${S08_CSV_LOG}"
      fi
