...
F21_cyclonedx_sbom() {
  ...
  if [[ -f "${S08_CSV_LOG}" ]] && [[ "$(wc -l "${S08_CSV_LOG}" | awk '{print $1}')" -gt 1 ]]; then
    ...
    while IFS=";" read -r COL1 COL2 COL3 COL4 COL5 COL6 COL7 COL8 COL9 COL10; do
      print_output "[*] Generating SBOM entry: ${COL1} - ${COL2} - ${COL3} - ${COL4} - ${COL5} - ${COL6} - ${COL7} - ${COL8} - ${COL9} - ${COL10}" "no_log"
      lTYPE="${COL1}"
      ...
      lCPE=""
      lPURL=""
      if grep -q ";${lBIN_NAME}\;${lSTRIPPED_VER};.*;${lCHKSUM};" "${F21_CSV_LOG}"; then
      ...
      lDESCRIPTION="SBOM generated by EMBA - type ${COL1} - originally identified version ${lVERSION}. Further description: ${lDESC}"
      write_csv_log "${lTYPE}" "${lMIME_TYPE}" "${lSUPPLIER}" "${lMAINTAINER}" "" "${COL1}" "${lBIN_NAME}" "${lSTRIPPED_VER:-NA}" "" "" "${lLICENSE:-NA}" "" "${lCPE}" "${lPURL}" "" "" "" "" "" "" "" "" "" "" "" "" "" "${lCHKSUM}" "" "" "" "" "" "" "" "" "${lDESCRIPTION}"
