...
F21_cyclonedx_sbom() {
  ...
  if [[ -f "${S08_CSV_LOG}" ]] && [[ "$(wc -l "${S08_CSV_LOG}" | awk '{print $1}')" -gt 1 ]]; then
    ...
    while IFS=";" read -r COL1 COL2 COL3 COL4 COL5 COL6 COL7; do
      ...
      lCPE=""
      lPURL=""
      write_csv_log "${lTYPE}" "${lMIME_TYPE}" "${lSUPPLIER}" "" "" "" "${lBIN_NAME}" "${lVERSION:-NA}" "" "" "${lLICENSE:-NA}" "" "${lCPE}" "${lPURL}" "" "" "" "" "" "" "" "" "" "" "" "" "" "${lCHKSUM}" "" "" "" "" "" "" "" "" "SBOM generated by EMBA - type ${COL1}"
    done < <(tail -n +2 "${S08_CSV_LOG}")
