...
ask_chatgpt() {
  ...
  for (( ELE_INDEX=0; ELE_INDEX<"${#Q02_OPENAI_QUESTIONS[@]}"; ELE_INDEX++ )); do
    ...
    if [[ "${SCRIPT_PATH_TMP_}" == *"s16_ghidra_decompile_checks"* ]]; then
      ...
      # we only do the complete gpt analysis if we have already found something with semgrep
      print_output "[*] Ghidra decompiled code found ${SCRIPT_PATH_TMP_}" "no_log"
      # SCRIPT_NAME is now the function name of the binary
      local SCRIPT_NAME=$(basename "${SCRIPT_PATH_TMP_}")
      if ! grep -q "Identified source function: .*${SCRIPT_NAME}" "${LOG_DIR}"/s16_ghidra_decompile_checks/semgrep_*; then
        print_output "[*] No s16 decompiled source function analysis found for ${ORANGE}${SCRIPT_NAME}${NC}" "no_log"
        # in case we have not already tested it with semgrep and have not found any results we can move on
        continue
      fi
