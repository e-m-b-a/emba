...
populate_karrays() {
  mapfile -t KERNEL_MODULES < <( eval find "$FIRMWARE_PATH" "${EXCL_FIND[@]}" -xdev -iname "*.ko" -type f "${FIND_UNIQUE_EXEC[@]}" )
...
check_modprobe()
{
  ...
  readarray -t MODPROBE_D_DIRS < <( eval find "$FIRMWARE_PATH" -xdev "${EXCL_FIND[@]}" -iname '*modprobe.d*' "${FIND_UNIQUE_EXEC[@]}" )
  for MP_DIR in "${MODPROBE_D_DIRS[@]}"; do
    if [[ -d "$MP_DIR" ]] ; then
      MP_CHECK=1
      print_output "[+] Found ""$(print_path "$MP_DIR")"
      readarray -t MODPROBE_D_DIR_CONTENT <<< "$( eval find "$MP_DIR" -xdev -iname '*.conf' "${FIND_UNIQUE_EXEC[@]}" )"
