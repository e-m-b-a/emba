...
check_fstab()
{
  ...
  if [[ ${#FSTAB_ARR[@]} -gt 0 ]] ; then
    readarray -t FSTAB_USER_FILES < <(printf '%s' "$(find "${FSTAB_ARR[@]}" "${EXCL_FIND[@]}" -xdev -print0|xargs -0 -P 16 -I % sh -c 'grep "username" % 2>/dev/null' || true)")
    readarray -t FSTAB_PASS_FILES < <(printf '%s' "$(find "${FSTAB_ARR[@]}" "${EXCL_FIND[@]}" -xdev -print0|xargs -0 -P 16 -I % sh -c 'grep "password" % 2>/dev/null' || true)")
  fi
