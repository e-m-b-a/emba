...
I13_disasm() {
  ...
  if [[ "${LIST_DEP}" -eq 1 ]] || [[ "${IN_DOCKER}" -eq 1 ]] || [[ "${DOCKER_SETUP}" -eq 0 ]] || [[ "${FULL}" -eq 1 ]]; then
    ...
    BINUTIL_VERSION_NAME="binutils-2.46.1"
    CAPA_VERSION="9.2.1"
    ...
    y | Y)
      ...
      if ! [[ -f "external/capa" ]]; then
        ...
        unzip external/capa-v"${CAPA_VERSION}"-linux.zip -d external || {
          echo "Could not install EMBA component Capa"
          exit 1
        }
      ...
      if ! [[ -f "external/objdump" ]]; then
        ...
        if [[ -f "external/${BINUTIL_VERSION_NAME}.tar.gz" ]]; then
          ...
          cd external/"${BINUTIL_VERSION_NAME}"/ || {
            echo "Could not install EMBA component binutils"
            exit 1
          }
      ...
      cd external/radare2 || {
        echo "Could not install EMBA component radare2"
        exit 1
      }
