  ...
  if [[ "${LIST_DEP}" -eq 1 ]] || [[ "${IN_DOCKER}" -eq 1 ]] || [[ "${DOCKER_SETUP}" -eq 0 ]] || [[ "${FULL}" -eq 1 ]]; then
    ...
    y | Y)
      ...
      if [[ -d "/home/linuxbrew/.linuxbrew/bin" ]]; then
        ...
        sudo -u linuxbrew NONINTERACTIVE=1 HOMEBREW_NO_SANDBOX_LINUX=1 /home/linuxbrew/.linuxbrew/bin/brew install cyclonedx/cyclonedx/cyclonedx-cli || { echo "Could not install EMBA component cyclonedx-cli via brew"; exit 1; }
