    ...
    case ${ANSWER:0:1} in
      y|Y )
        ...
        if ! [[ -d ./external/cwe_checker ]]; then
          ...
          curl -L https://github.com/NationalSecurityAgency/ghidra/releases/download/Ghidra_10.1.5_build/ghidra_10.1.5_PUBLIC_20220726.zip -Sf -o external/ghidra.zip
          mkdir external/ghidra 2>/dev/null
          unzip -qo external/ghidra.zip -d external/ghidra
          if [[ "$IN_DOCKER" -eq 1 ]]; then
            sed -i s@JAVA_HOME_OVERRIDE=@JAVA_HOME_OVERRIDE=/external/jdk@g external/ghidra/ghidra_10.1.5_PUBLIC/support/launch.properties
          else
            sed -i s@JAVA_HOME_OVERRIDE=@JAVA_HOME_OVERRIDE=external/jdk@g external/ghidra/ghidra_10.1.5_PUBLIC/support/launch.properties
          fi
          ...
          if [[ "$IN_DOCKER" -eq 1 ]]; then
            ...
            echo '{"ghidra_path":"/external/ghidra/ghidra_10.1.5_PUBLIC"}' > /root/.config/cwe_checker/ghidra.json
