...
check_path_valid() {
  C_PATH="$1"
  if [[ -n "$C_PATH" ]] && ( [[ "${C_PATH:0:2}" != "./" ]] && [[ "${C_PATH:0:2}" != "./" ]] && [[ "${C_PATH:0:3}" != "../" ]] ) ; then
    print_output "[!] ""$C_PATH"" ""${C_PATH:0:1}"" ""${C_PATH:0:2}"" ""${C_PATH:0:3}"" "" is not a valid path in the context of emba" "no_log"
