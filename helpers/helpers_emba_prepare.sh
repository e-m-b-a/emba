...
detect_root_dir_helper() {
  ...
  if [[ "${SBOM_MINIMAL:-0}" -eq 0 ]]; then
    mapfile -t lINTERPRETER_FULL_PATH_ARR < <(find "${lSEARCH_PATH}" -ignore_readdir_race -type f -exec file -b {} \; 2>/dev/null | grep "ELF.*interpreter" | sed s/.*interpreter\ // | sed 's/,\ .*$//' | grep "^/" | sort -u 2>/dev/null || true)
