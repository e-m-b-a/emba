...
prepare_file_arr() {
  ...
  export FILE_ARR=()
  readarray -t FILE_ARR < <(find "${lFIRMWARE_PATH}" -xdev "${EXCL_FIND[@]}" -type f -print0|xargs -0 -P 16 -I % sh -c 'md5sum %' | sort -u -k1,1 | cut -d\  -f3- )
  ...
  if [[ -f ${lFIRMWARE_PATH} && ${RTOS} -eq 1 ]]; then
    ...
    readarray -t FILE_ARR_RTOS < <(find "${OUTPUT_DIR}" -xdev -type f -print0|xargs -0 -P 16 -I % sh -c 'md5sum %' | sort -u -k1,1 | cut -d\  -f3- )
...
prepare_binary_arr() {
  ...
  readarray -t lBINARIES_TMP_ARR < <(find "${lFIRMWARE_PATH}" "${EXCL_FIND[@]}" -type f -print0|xargs -0 -P 16 -I % sh -c 'file % | grep "ELF\|PE32" | cut -d: -f1' || true)
...
prepare_file_arr_limited() {
  ...
  readarray -t FILE_ARR_LIMITED < <(find "${lFIRMWARE_PATH}" -xdev "${EXCL_FIND[@]}" -type f ! \( -iname "*.udeb" -o -iname "*.deb" \
    ...
    -print0|xargs -0 -P 16 -I % sh -c 'md5sum % 2>/dev/null || true' | sort -u -k1,1 | cut -d\  -f3-)
...
detect_root_dir_helper() {
  ...
  if [[ "${SBOM_MINIMAL:-0}" -eq 0 ]]; then
    ...
    mapfile -t lINTERPRETER_FULL_PATH_ARR < <(find "${lSEARCH_PATH}" -ignore_readdir_race -type f -print0|xargs -0 -P 16 -I % sh -c 'file -b % | grep "ELF" | grep "interpreter" | sed s/.*interpreter\ // | sed s/,\ .*$//' | sort -u 2>/dev/null || true)
    ...
    mapfile -t lROOTx_PATH_ARR < <(find "${lSEARCH_PATH}" -xdev -path "*bin/bash" -print0|xargs -0 -P 16 -I % sh -c 'file % | grep "ELF" | cut -d: -f1 | sed -E s/\/.?bin\/bash//' || true)
    for lR_PATH in "${lROOTx_PATH_ARR[@]}"; do
      if [[ -d "${lR_PATH}" ]]; then
        ROOT_PATH+=( "${lR_PATH}" )
        if [[ -z "${lMECHANISM}" ]]; then
    ...
    mapfile -t lROOTx_PATH_ARR < <(find "${lSEARCH_PATH}" -xdev -path "*bin/sh" -print0|xargs -0 -P 16 -I % sh -c 'file % | grep "ELF" | cut -d: -f1 | sed -E s/\/.?bin\/sh//' || true)
