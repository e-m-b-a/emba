  ...
  if [[ "${USE_DOCKER}" -eq 0 ]] ; then
    ...
    if [[ "${CONTAINER_NUMBER}" -ne 2 ]]; then
      ...
      if [[ "${FULL_EMULATION}" -eq 1 ]]; then
        ...
        check_dep_file "vmlinux.*" "${EXT_DIR}""/EMBA_Live_bins/Linux-Kernel-v${L10_KERNEL_V_LONG}/vmlinux.mipseb.4"
        check_dep_file "zImage.arm*" "${EXT_DIR}""/EMBA_Live_bins/Linux-Kernel-v${L10_KERNEL_V_LONG}/zImage.armel"
