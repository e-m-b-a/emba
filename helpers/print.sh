...
print_help()
{
  ...
  echo -e "$CYAN""-e [./path]""$NC""       Excludes paths from testing (multiple usage possible)"
  echo -e "$CYAN""-m [MODULE_NO.]""$NC""   Tests only with set modules [e.g. -m p05 -m s10 ... ]]"
  ...
  echo -e "$CYAN""-c""$NC""                Enables cwe-checker"
  ...
  echo -e "$CYAN""-x""$NC""                Enable deep extraction - try to extract every file two times with binwalk (WARNING: Uses huge disk space)"
  echo -e "\\nModify output"
